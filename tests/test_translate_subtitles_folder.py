...
def test_translate_all_srt_files(tmp_path, monkeypatch):
    ...
    # Intercept calls to process_srt_file and translate_srt_file to avoid real work
    proc_calls = []
    ...
    monkeypatch.setattr(translate_subtitles_folder, 'process_srt_file', fake_process)

    trans_calls = []
    def fake_translate(input_file, output_file, slang_file, chunk_size, overlap, cache_dir, model):
        trans_calls.append((input_file, output_file, slang_file, chunk_size, overlap, cache_dir, model))
        os.makedirs(os.path.dirname(output_file), exist_ok=True)
        Path(output_file).write_text("translated", encoding='utf-8')
    monkeypatch.setattr(translate_subtitles_folder, 'translate_srt_file', fake_translate)
