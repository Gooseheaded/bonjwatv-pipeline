...
from google_sheet_write import run_google_sheet_write
...
def test_google_sheet_write(tmp_path, caplog, patch_gspread):
    ...
    cache_file.write_text(
        json.dumps({"url": "https://pastebin.com/raw/ABC"}), encoding="utf-8"
    )

    caplog.set_level("INFO")
    # Run update
    assert run_google_sheet_write(
...
def test_skip_missing_id(tmp_path, caplog, patch_gspread):
    ...
    cache_file.write_text(
        json.dumps({"url": "https://pastebin.com/raw/XYZ"}), encoding="utf-8"
    )
