...
from build_videos_json import run_build_videos_json
...
def test_build_videos_json_enriches_fields(tmp_path):
    ...
    (metadata_dir / "id1.json").write_text(
        json.dumps({"title": "원제목", "uploader": "Creator1"}), encoding="utf-8"
    )
    (cache_dir / "title_id1.json").write_text(
        json.dumps({"title_en": "Translated Title", "source_hash": "x"}),
        encoding="utf-8",
    )
    # id2 has metadata but no title cache; EN Title preset should remain
    (metadata_dir / "id2.json").write_text(
        json.dumps({"title": "다른 제목", "uploader": "Creator2"}), encoding="utf-8"
    )

    assert run_build_videos_json(str(video_list), str(metadata_dir), str(cache_dir))
