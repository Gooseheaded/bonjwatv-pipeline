...
from upload_subtitles import run_upload_subtitles
...
def test_upload_subtitles(tmp_path, monkeypatch, caplog):
    ...
    ok = run_upload_subtitles(str(input_srt), str(cache_dir))
    assert ok is True
    # Expect raw paste URL
    url = json.loads((cache_dir / "pastebin_vid.json").read_text(encoding="utf-8"))["url"]
...
def test_skip_cached(tmp_path, caplog):
    ...
    cache_file.write_text(
        json.dumps({"paste_id": "XYZ", "url": "http://cached"}), encoding="utf-8"
    )

    caplog.set_level("INFO")
    ok = run_upload_subtitles(str(input_srt), str(cache_dir))
    assert ok is True
    url = json.loads(cache_file.read_text(encoding="utf-8"))["url"]
...
def test_login_and_cache_user_key(tmp_path, monkeypatch, caplog):
    ...
    calls.clear()
    ok2 = run_upload_subtitles(str(input_srt), str(cache_dir))
    assert ok2 is True
    assert calls == []
    assert "Using cached Pastebin URL" in caplog.text
...
def test_api_returns_full_url(tmp_path, monkeypatch, caplog):
    ...
    url = json.loads((cache_dir / "pastebin_vid.json").read_text(encoding="utf-8}"))
    url = url["url"]
