...
from fetch_video_metadata import fetch_video_metadata
...
def test_fetch_video_metadata_skips_if_exists(tmp_path):
    ...
    fetch_video_metadata(video_id, str(output_dir))
...
def test_fetch_video_metadata_fetches_and_saves(mock_yt_dlp, tmp_path):
    ...
    fetch_video_metadata(video_id, output_dir)

    mock_yt_dlp.extract_info.assert_called_once_with(f"https://www.youtube.com/watch?v={video_id}", download=False)
