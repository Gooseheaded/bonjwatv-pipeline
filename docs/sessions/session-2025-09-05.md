...
## Production Incident — Search Returned No Results (Evening)
...
- The API therefore served an empty catalog while the webapp homepage still had its own baked `data/videos.json` → Search (API-backed) returned 0 items.
...
- Added integration tests:
  - `Reject_Deletes_Unreferenced_Subtitle` — file removed after reject when not referenced.
  - `Reject_DoesNot_Delete_When_Referenced_In_Videos` — file retained when `videos.json` references it.
- All tests pass.

### Ratings UX Improvements
- Watch page:
  - Highlights the current user’s rating; clicking the selected rating removes it (new DELETE endpoint).
  - Webapp forwards identity headers on GET/POST/DELETE so the API can return `UserRating`.
- Search page: shows a compact ratings summary (🔴 🟡 🟢) on each card.
...
- API: `DELETE /api/videos/{id}/ratings` removes the current user’s rating and decrements aggregates.
- Tests: added `Ratings_Remove_Clears_User_And_Counts` to verify POST→GET (with user)→DELETE→GET flow.

### Homepage Ratings & Sorting
- Homepage cards show a compact ratings summary (🔴 🟡 🟢) like the Search page.
- Homepage sorts videos by a Wilson lower bound score of a “positive” rate where Positive = Green + 0.5 × Yellow and Total = Red + Yellow + Green (z = 1.96). Ties break by total votes, then title.
- Introduced `IRatingsClient` abstraction; default `HttpRatingsClient` calls the Catalog API. Tests inject a fake client to avoid network and verify ordering deterministically.
- Test added to assert order (C → B → A) based on injected ratings.

### Translator GUI Updates
- Renamed section to “bonjwa.tv integration”, moved it below Pipeline steps.
- Renamed label to “bonjwa.tv URL” with default `https://bonjwa.tv`.
- Disabled URL/token inputs until “Submit results to bonjwa.tv” is checked.
