...
- Hardened hidden-video updates by adding a `ForceReload()` hook on `VideoRepository` and calling it after hide/show/tag/duration/delete mutations (and after submission approvals) so the admin UI sees changes immediately despite filesystem timestamp resolution quirks.
