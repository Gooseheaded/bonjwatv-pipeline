...
- Added Docker packaging + deploy flow:
  ...
  - Updated prod tagging: prod compose now uses `bwkt-webapp:prod` and `bwkt-catalog-api:prod`, and the packaging script saves those tags.
...
- Catalog API reads from `Data:JsonPath` (or `DATA_JSON_PATH`) and exposes `/api/videos` with `q`, `race`, and paging. Forwarded headers enabled for reverse proxies.
- Webapp now honors `DATA_CATALOG_URL`: when set, search results are fetched from the API (with race filtering server-side) and fall back to local JSON on failure; otherwise, local search path is used.

## Testing

- Added CatalogApi.Tests using `WebApplicationFactory<Program>` to validate `/api/videos` query, race filter, and pagination against a temp JSON dataset.
- Expanded webapp tests:
  - API fallback test validates `DATA_CATALOG_URL` usage with graceful fallback.
  - Search page passes normalized race to service; added a test for this behavior.
  - Updated badge markup test to match theme-aware layout (`me-1`).
- Hardened SearchModel for tests: guards cookie read/write when `HttpContext` is null.
- All tests pass locally (`dotnet test`), aside from benign MSBuild copy warnings.

## Repo Hygiene

- Added a root `.gitignore` to exclude `.NET` build artifacts (`**/bin`, `**/obj`), local `dist/` bundles, and test logs.
- Removed accidentally tracked `catalog-api/obj` files from version control.
