...
- Translator fixes:
  ...
  - OpenAI STT 429 (insufficient_quota): Detect and mark quota as blocked; skip remaining transcriptions in the run to avoid repeated failures.
...
## Webapp Admin UX
- Added a quick Reject button next to “Open” in the Admin index Pending Submissions table. Approval still requires opening the detail page to review subtitle preview.
- After approval, the success banner includes a direct “Watch now” link. The server fetches `youtube_id` before applying approval and redirects with `?v=<id>` so the UI can render the link.

## Webapp Perf/UX
- Reduced white flash on navigation by setting early background colors in the HTML head (light: `#f8f9fa`, dark: `#212529`) and applying the saved theme before first paint. This avoids a bright flash while the page reloads.
## Decisions
